...
func (f *FabricClient) QueryChaincode(peerName string, args []string) ([]byte, error) {
	...
	return f.runPeerCommand(peerName, []string{
		"chaincode", "query",
		"-C", f.cfg.Channel,
		"-n", f.cfg.Chaincode,
		"-c", MustJSON(payload),
...
func (f *FabricClient) InvokeChaincode(peerName string, args []string) error {
	payload := map[string]any{"Args": args}
	_, err := f.runPeerCommand(peerName, []string{
		"chaincode", "invoke",
...
func (f *FabricClient) runPeerCommand(peerName string, args []string) ([]byte, error) {
	...
	env := append(os.Environ(),
		...
		fmt.Sprintf("CORE_PEER_MSPCONFIGPATH=%s", f.cfg.MSPPath),
